...
def fetch_github_contributions(username="octocat"):
    """Fetch the real contribution calendar using GitHub GraphQL.

    This avoids scraping GitHub's dynamic HTML, which is the reason the old
    implementation could fall back to SYNC even when Actions itself succeeded.
    """
    ...
    if token:
        ...
        try:
            ...
            return {
                "days": days,
                "total_year": f'{calendar["totalContributions"]:,}'
            }
        except Exception as e:
            print(f"[Contributions] GraphQL notice: {e}")

    # Legacy HTML fallback for local/manual runs without a token.
    url = f"https://github.com/users/{username}/contributions"
    headers = {"User-Agent": "mouli-github-profile"}
    req = urllib.request.Request(url, headers=headers)
    try:
        with urllib.request.urlopen(req, timeout=12) as resp:
            html = resp.read().decode("utf-8")
    except Exception as e:
        print(f"[Contributions] Notice: Could not fetch live calendar ({e}).")
        return None
    ...
    total_text = total_match.group(1) if total_match else f"{sum(d['count'] for d in days):,}"

    return {"days": days, "total_year": total_text}
...
def generate_contributions_svg(config_path="config.yml", output_path="assets/contributions.svg"):
    ...
    if contrib_data and contrib_data.get("days"):
        ...
        total_year = "SYNC"
        recent_days = [{"date": f"Day {i}", "count": 0} for i in range(31)]
